...
def test_actor_class_method(ray_start_regular_shared):
    ...
    # assert ray.get(Parent.testing.remote(1)) == 16
    pg = ray.util.placement_group(bundles=[{"CPU": 1}])
    ray.get(pg.ready())
    a = Actor.options(
            scheduling_strategy=ray.util.scheduling_strategies.PlacementGroupSchedulingStrategy(placement_group=pg),
            name="testing",
        ).remote()
    
    
    # s = ray._private.worker.global_worker.core_worker.get_reference_table_state()
    # print("STTATEEE", s)

    assert ray.util.list_named_actors() == ["testing"]
